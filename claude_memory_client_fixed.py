...
def extract_entities_from_text(text: str) -> List[str]:
    ...
    Looks for single uppercase letters that represent people/entities,
    being careful to avoid false positives from articles and common words.
    """
    entities = []
    
    # Pattern for single uppercase letter as standalone entity
    # Must be preceded by space/start and followed by space/punctuation/end
    # This avoids matching "A" in "A dog" but catches "relationship with A"
    entity_pattern = r'(?:^|\s)([A-Z])(?=\s|[,.:;!?]|$)'
    
    matches = re.findall(entity_pattern, text)
    for letter in matches:
        # Exclude common articles/words that are single letters
        if letter not in ['I']:  # 'I' is almost always the pronoun
            # Additional context check: if preceded by "a" or "an", likely an article
            context_pattern = rf'\b(?:a|an)\s+{letter}\b'
            if not re.search(context_pattern, text, re.IGNORECASE):
                entities.append(letter)
    ...
    # Common entity patterns (not single letters)
    if any(word in text.lower() for word in ["trauma", "triggered", "activation"]):
        if "trauma_responses" not in unique_entities:
            unique_entities.append("trauma_responses")
